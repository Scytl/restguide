...
public class PersistenceExceptionMapper implements ExceptionMapper<PersistenceException> {
  ...
  public Response toResponse(PersistenceException exception) { // <2>
    return Response.status(Status.INTERNAL_SERVER_ERROR)
          .entity(exception.getMessage()).type("text/plain").build(); // <3>
  }
}
----
<1> Mapper should be annotated with +@Provider+ annotation.
<2> +toResponse+ method builds the response to be sent back to caller.
<3> Along with _HTTP_ code, a message about the error is also returned.
...
Because it returns a subresource, the engine will continue inspecting the left part of the _URI_, in this case `/chapters`, through returned subresource.
...
TIP: You can use the same approach to return single attribute in case client needs to access them from your _REST_ API. It is not necessary to use suubresources only in case of _Collections_.

=== Versioning

As seen in <<versioning-rest, REST versioning>> 
