...
To return all _chapters_ given _book_, you can do something like:
...
It is better to maintain classes as simple as possible, and more important doing the work they was designed for, in case of `BookResource` it is dealing with books and not chapters.

So one way is make `Book` as *sub resource*. Thanks of subresources, all RESTful endpoint matching releated with returning a list of chapters will be delegated to `Book`.
