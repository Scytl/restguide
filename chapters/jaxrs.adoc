...
----
<1> `@Path` sets a class as REST endpoint. In this case is bound to `/book`.
...
Then we can access this resource in `http://<host>:<port>/<application>/book`.
...
We can access this resource in `http://<host>:<port>/<application>/book/123`.
...
Resource in `http://<host>:<port>/<application>/book/123` will work but `http://<host>:<port>/<application>/book/123` returns an _HTTP_ 404 code error.
====
...
We can access this resource in `http://<host>:<port>/<application>/book?orderBy=name`.
...
Now instead of accessing the resource through `http://<host>:<port>/<application>/book`, we need to go to `http://<host>:<port>/<application>/public/book`.
...
We can access this resource in `http://<host>:<port>/<application>/book/123/chapter` and the list of chapters will be returned.
...
First of all it will run `http://<host>:<port>/<application>/book/123` in `BookResource`.
...
So it will find that `Book` class contains a method annotated with `@Path("/chapter")`, and because it matches, the method will be executed by returning a list of chapters.

TIP: You can use the same approach to return single attribute in case client needs to access them from your _REST_ API, for example `/book/title`.
[TIP]
====
...
Versioning can be implemented following different approaches, but using <<Sub-resource locator>> may be cleaner way to implement it.

Let's suppose we have different resources and we want to give support to two different versions '`v1`' and '`v2`'.
The first thing to do is create two sub-resources, one for each version.
The main feature of these sub-resources are dispatching to required business logic.

[source, java]
.DispatcherResourceV1.java
----
@Path("/v1") //<1>
public class DispatcherResourceV1 {

  @Path("/chapter")
  public ChapterResourceV1 v1Chapters() { //<2>
    return new ChapterResourceV1(); //<3>
  }
...
<2> It returns the chapter resource that contains the implementation of version 1.
<3> Resource is created each time, but you can write a _singleton_ factory or create it as attribute parameter in sub-resource class.
<4> And the same for all resources available in version 1 of the _API_.
...
[source, java]
.DispatcherResourceV2.java
----
@Path("/v2") //<1>
public class DispatcherResourceV2 {

  @Path("/chapter")
  public ChapterResourceV2 v2Chapters() { //<2>
    return new ChapterResourceV2();
  }
...
<2> Now the second version of the _Chapter_ resource is returned.
<3> If other resources has not changed you can return the same resource as before.
